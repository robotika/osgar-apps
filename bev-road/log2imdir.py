...
def main():
    parser = argparse.ArgumentParser(
        description="Extract camera frames from OSGAR logs at specific distance intervals"
    )
    parser.add_argument("logfile", help="Path to the input OSGAR log file")
    parser.add_argument(
        "-o", "--out",
        help="Output directory (defaults to logfile name without extension)"
    )
    parser.add_argument(
        "--start", type=float, default=1.0,
        help="Start distance in meters (default: 1.0)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Camera stream name (default: 'oak.color')"
    )
    parser.add_argument(
        "--pose", default="platform.pose2d",
        help="Pose2D stream name (default: 'platform.pose2d')"
    )
    ...
    extract_images_from_reader(
        reader, output_dir, args.start, args.step, args.end, args.camera, args.pose
    )
            ...
            if stream_name == camera_stream:
                ...
                while accumulated_dist >= target_dist and target_dist <= end:
                    if last_frame is not None:
                        ...
                        print(
                            f"Extracted {img_filename} at distance "
                            f"{accumulated_dist:.2f}m (target {target_dist:.2f}m)"
                        )
                        img_idx += 1
                    else:
