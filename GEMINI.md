...
1.  **Local Replay Verification:** Run `osgar.replay` on the robot's log locally. Verify that the behavior (outputs/prints) matches what was seen on the robot. If you have modified the configuration file (e.g., added new links or parameters), you MUST provide it via the `--config` flag. This confirms the environment is correct and the issue is reproducible.
