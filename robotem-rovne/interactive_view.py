...
def load_log_data(logfile):
    ...
    # Target height for visualization - decreased slightly to fit better with UI
    TARGET_H = 600
...
def render_frame(window_name):
    ...
    if val >= len(state['frames']): return
...
def main():
    ...
    logfile = sys.argv[1]
    state['frames'] = load_log_data(logfile)
    ...
    while True:
        # waitKeyEx catches arrow keys on Windows more reliably
        key = cv2.waitKeyEx(0)
        ...
        # Keys: a/d or arrows (81/83 or large codes)
